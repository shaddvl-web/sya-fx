...
class SyaChart {
  ...
  setToggles(toggles) {
    ...
    if (toggles.showPatterns !== undefined) this.showPatterns = toggles.showPatterns;
    if (toggles.showSignals !== undefined) this.showSignals = toggles.showSignals;
  ...
  initEvents() {
    ...
    this.canvas.addEventListener('touchmove', (e) => {
      if (e.touches.length === 1 && this.isDragging) {
        ...
        if (Math.abs(diff) > 8) {
          if (diff > 0) {
            this.visibleBars = Math.max(this.minBars, this.visibleBars - 2);
          } else {
            this.visibleBars = Math.min(this.maxBars, Math.min(this.candles.length, this.visibleBars + 2));
          }
          touchStartDist = dist;
          this.render();
        }
  ...
  render() {
    ...
    // 6b. Draw Chart Patterns (Boxes, Projections, Kurdish Labels)
    if (this.showPatterns && this.analysis?.patterns && this.analysis.patterns.length > 0) {
      this.drawPatterns(ctx, visibleCandles, startIndex, barWidth, priceToY, candleBodyWidth);
    }

    // 6c. Draw AI Buy/Sell Signals directly on Candles
    if (this.showSignals && this.analysis?.signal) {
      this.drawBuySellSignals(ctx, visibleCandles, startIndex, barWidth, priceToY, candleBodyWidth);
    }

    // 6d. Draw Floating Institutional AI Decision HUD Badge
    if (this.showSignals && this.analysis?.signal) {
      this.drawAiDecisionBadge(ctx, mainChartWidth, priceToY);
    }
  ...
  getPatternKurdishName(p) {
    if (p.kurdishName) return p.kurdishName;
    const map = {
      'Bullish Engulfing': 'مۆمی داپۆشەری کڕین (Bullish Engulfing)',
      'Bearish Engulfing': 'مۆمی داپۆشەری فرۆشتن (Bearish Engulfing)',
      'Bullish Pin Bar / Hammer': 'مۆمی چەکوش (Hammer)',
      'Shooting Star / Bearish Pin Bar': 'نەیزەکی فرۆشتن (Shooting Star)',
      'Morning Star': 'ئەستێرەی بەیانی (Morning Star)',
      'Double Bottom (W-Formation)': 'دووانە بنکە (Double Bottom - W)',
      'Double Top (M-Formation)': 'دووانە لووتکە (Double Top - M)',
      'Momentum Continuation Base': 'بەردەوامی ترێند (Continuation)'
    };
    return map[p.pattern] || p.pattern;
  }

  drawPatterns(ctx, visibleCandles, startIndex, barWidth, priceToY, candleBodyWidth) {
    if (!this.analysis || !this.analysis.patterns) return;
    const isKu = !window.I18n || window.I18n.currentLang === 'ku';

    this.analysis.patterns.forEach((p) => {
      const localStart = p.startIndex - startIndex;
      const localEnd = p.endIndex - startIndex;

      // Check if pattern intersects visible viewport
      if (localEnd < 0 || localStart >= visibleCandles.length) return;

      const cStart = Math.max(0, localStart);
      const cEnd = Math.min(visibleCandles.length - 1, localEnd);
      ...
      const x1 = cStart * barWidth + 2;
      const x2 = (cEnd + 1) * barWidth - 2;
      const boxW = Math.max(candleBodyWidth * 1.6, x2 - x1);

      const yTop = priceToY(pHigh) - 10;
      const yBottom = priceToY(pLow) + 10;
      const boxH = Math.max(26, yBottom - yTop);

      const isBull = p.direction === 'BULLISH';
      const colorAccent = isBull ? '#00ffa3' : '#ff3d71';
      const bgFill = isBull ? 'rgba(0, 255, 163, 0.08)' : 'rgba(255, 61, 113, 0.08)';
      const borderStroke = isBull ? 'rgba(0, 255, 163, 0.65)' : 'rgba(255, 61, 113, 0.65)';

      ctx.save();

      // 1. Shaded Region for Pattern Candles
      ...
      // 3. Crisp Precision Corner Brackets
      ...
      // Top-Left
      ctx.beginPath();
      ctx.moveTo(x1, yTop + cornerLen);
      ctx.lineTo(x1, yTop);
      ctx.lineTo(x1 + cornerLen, yTop);
      ctx.stroke();

      // Top-Right
      ctx.beginPath();
      ctx.moveTo(x1 + boxW - cornerLen, yTop);
      ctx.lineTo(x1 + boxW, yTop);
      ctx.lineTo(x1 + boxW, yTop + cornerLen);
      ctx.stroke();

      // Bottom-Left
      ctx.beginPath();
      ctx.moveTo(x1, yBottom - cornerLen);
      ctx.lineTo(x1, yBottom);
      ctx.lineTo(x1 + cornerLen, yBottom);
      ctx.stroke();

      // Bottom-Right
      ctx.beginPath();
      ctx.moveTo(x1 + boxW - cornerLen, yBottom);
      ctx.lineTo(x1 + boxW, yBottom);
      ctx.lineTo(x1 + boxW, yBottom - cornerLen);
      ctx.stroke();

      // 4. Pattern Label Tag (Badge)
      const nameText = isKu ? this.getPatternKurdishName(p) : `${p.pattern} (${p.direction})`;
      const labelText = `${isBull ? '▲' : '▼'} ${nameText} • ${p.confidence}%`;

      ctx.font = 'bold 10px "Segoe UI", Tahoma, SF Mono, sans-serif';
      const textMetrics = ctx.measureText(labelText);
      const tagW = Math.max(boxW, textMetrics.width + 16);
      const tagH = 20;
      const tagX = Math.max(4, Math.min(this.width - 80 - tagW, x1 + boxW / 2 - tagW / 2));
      const tagY = isBull ? Math.max(16, yTop - tagH - 5) : Math.min(this.height - 120, yBottom + 5);
      ...
      // 5. Target Projection Line
      if (p.targetPrice) {
        const yTarget = priceToY(p.targetPrice);
        const lineStartX = x1 + boxW;
        const lineEndX = Math.min(this.width - 76, lineStartX + 120);

        ctx.strokeStyle = colorAccent;
        ctx.lineWidth = 1;
        ctx.setLineDash([3, 3]);
        ctx.beginPath();
        ctx.moveTo(lineStartX, yTarget);
        ctx.lineTo(lineEndX, yTarget);
        ctx.stroke();
        ctx.setLineDash([]);

        // Target Tag
        ctx.fillStyle = colorAccent;
        ctx.font = '9px SF Mono, sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(`🎯 ${isKu ? 'ئامانج' : 'TP'}: ${p.targetPrice.toFixed(5)}`, lineEndX + 4, yTarget + 3);
      }
  ...
  drawBuySellSignals(ctx, visibleCandles, startIndex, barWidth, priceToY, candleBodyWidth) {
    if (!this.analysis?.signal) return;
    const sig = this.analysis.signal;
    const isKu = !window.I18n || window.I18n.currentLang === 'ku';
    ...
    // Position marker on the last candle
    const lastIdx = visibleCandles.length - 1;
    if (lastIdx < 0) return;
    const c = visibleCandles[lastIdx];
    const x = lastIdx * barWidth + barWidth / 2;

    ctx.save();

    if (isBuy) {
      // Upward Arrow below low
      const lowY = priceToY(c.low);
      const arrowTipY = lowY + 8;
      const arrowBaseY = lowY + 22;
      ...
      ctx.lineTo(x - 7, arrowBaseY);
      ctx.lineTo(x + 7, arrowBaseY);
      ctx.closePath();
      ctx.fill();
      ctx.shadowBlur = 0;

      // Action Badge Pill below arrow
      const badgeText = isKu ? `▲ کڕین (BUY) | ${sig.finalScore.toFixed(0)}%` : `▲ BUY | ${sig.finalScore.toFixed(0)}%`;
      ctx.font = 'bold 10.5px "Segoe UI", Tahoma, SF Mono, sans-serif';
      const tw = ctx.measureText(badgeText).width;
      const bw = tw + 14;
      const bh = 19;
      const bx = Math.max(6, Math.min(this.width - 80 - bw, x - bw / 2));
      const by = arrowBaseY + 4;
      ...
      // Downward Arrow above high
      const highY = priceToY(c.high);
      const arrowTipY = highY - 8;
      const arrowBaseY = highY - 22;

      // Glowing Arrow
      ctx.fillStyle = '#ff3d71';
      ctx.shadowColor = 'rgba(255, 61, 113, 0.8)';
      ctx.shadowBlur = 10;
      ctx.beginPath();
      ctx.moveTo(x, arrowTipY);
      ctx.lineTo(x - 7, arrowBaseY);
      ...
      // Action Badge Pill above arrow
      const badgeText = isKu ? `▼ فرۆشتن (SELL) | ${sig.finalScore.toFixed(0)}%` : `▼ SELL | ${sig.finalScore.toFixed(0)}%`;
      ctx.font = 'bold 10.5px "Segoe UI", Tahoma, SF Mono, sans-serif';
      const tw = ctx.measureText(badgeText).width;
      const bw = tw + 14;
      const bh = 19;
      const bx = Math.max(6, Math.min(this.width - 80 - bw, x - bw / 2));
      const by = arrowBaseY - bh - 4;
  ...
  drawAiDecisionBadge(ctx, mainChartWidth, priceToY) {
    if (!this.analysis?.signal) return;
    const sig = this.analysis.signal;
    ...
    const sigText = isBuy
      ? (isKu ? 'کڕین (BUY)' : 'BUY')
      : (isSell ? (isKu ? 'فرۆشتن (SELL)' : 'SELL') : (isKu ? 'چاوەڕوانی (WAIT)' : 'WAIT'));

    const badgeW = Math.min(270, mainChartWidth - 20);
    const badgeH = 48;
    const badgeX = mainChartWidth - badgeW - 12;
    const badgeY = 12;

    if (badgeW < 140) return; // Don't crowd tiny viewports
    ...
    ctx.fillText(isKu ? 'شیکاری ژیری دەستکرد (AI):' : 'AI SIGNAL:', badgeX + 10, badgeY + 7);

    ctx.font = 'bold 12px "Segoe UI", Tahoma, sans-serif';
    ctx.fillStyle = sigColor;
    ctx.fillText(sigText, badgeX + (isKu ? 148 : 95), badgeY + 6);

    ctx.font = 'bold 10px SF Mono, monospace';
    ctx.fillStyle = '#ffffff';
    ctx.textAlign = 'right';
    ctx.fillText(`${sig.finalScore.toFixed(0)}%`, badgeX + badgeW - 8, badgeY + 7);
    ...
    if (risk && risk.entry) {
      ...
      const line2 = isKu
        ? `چوون: ${risk.entry.toFixed(5)} | SL: ${risk.stopLoss.toFixed(5)} | TP: ${risk.takeProfit1.toFixed(5)}`
        : `Entry: ${risk.entry.toFixed(5)} | SL: ${risk.stopLoss.toFixed(5)} | TP: ${risk.takeProfit1.toFixed(5)}`;
      ctx.fillText(line2, badgeX + 10, badgeY + 26);
    }
